...
// Debug: confirm env exists (safe prefix only)
console.log(
...
export async function POST(req: Request) {
  try {
    ...
    // 🔒 Server-side price computation (authoritative)
    ...
    // 🔐 Validate cash vs card amounts (single source of truth)
    ...
    await Booking.create({
      ...
      depositAmount: isCash ? chargeAmount : undefined,
      amountDue: isCash ? fullAmount - chargeAmount : 0,
    ...
    if (!isProd && err instanceof Error) {
      responseBody.details = { stack: err.stack };
    }
